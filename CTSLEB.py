...
# # CTSLEB
# 
# In this notebook we will use CTSLEB to caluclate the PRS.
# 
# Installation  open R using command prompt or terminal and install it
# 
# R
# 
...
# 
# 
# Download the html file to follow the tutorial from the author
# 
# https://github.com/andrewhaoyu/CTSLEB/blob/main/Tutorial_CTSLEB.html
# 
# This tool uses two gwas (same phenotype but different pipulation ) for the eastimating risk scores for the target population.
# 1. We considerd the target gwas to be the GWAS of the specific phenotyupe.
# 2. We considered the original gwaw to be of asthma phenotyupe. One can use GWAS for same phenotupe/disease for the different pipialtion diease. According to the tool documentation, the popation should be different but the disease should be the same. We used the difffernt gwas just to illustrate the process of using this tool. 
# 3. Note that both GWAS should be differnet otherwise the code may not work. 
# 4. One can pass different train and  test set, but we considered the same train and test set, as multiple gwas can have the same genotype data. 
# 5. 
# 
# 
# 
