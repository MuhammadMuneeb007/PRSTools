...
# 
# PRSbils uses LD reference panel for calculation: [PRSbils GitHub](https://github.com/styvon/PRSbils).
# 
# 
# 
# ## Download PRSbils
...
# |-------------------------|-----------------------------------------------------------------------------------------------------------------|-------------|
# | `--ref_dir REF_DIR`     | Full path to directory containing LD reference panel (snpinfo_1kg_hm3 and ldblk_1kg_chr*.hdf5)                   | -           |
# | `--bim_prefix BIM_PREFIX` | Full path and prefix of the bim file                                                                           | -           |
# | `--sst_file SST_FILE`   | Full path and name of GWAS summary statistics file                                                              | -           |
# | `--map_file MAP_FILE`   | Full path and name of the SNP to sets mapping file                                                              | -           |
# | `--map_gcol MAP_GCOL`   | Column id for group info in map_file (not yet implemented)                                                      | None        |
# | `--n_gwas N_GWAS`       | Sample size of the GWAS                                                                                         | -           |
# | `--out_dir OUT_DIR`     | Output directory and filename prefix for the posterior effect size estimates                                    | -           |
# | `--a A`                 | Parameter a for sigma                                                                                           | 0.001       |
# | `--b B`                 | Parameter b in the gamma prior for sigma                                                                        | 0.001       |
# | `--c C`                 | Parameter c for group implementation                                                                            | 0.001       |
# | `--d D`                 | Parameter d for group implementation                                                                            | 0.001       |
# | `--e E`                 | Parameter e for lambda                                                                                          | 1           |
# | `--f F`                 | Parameter f for lambda                                                                                          | 0.5         |
# | `--n_iter N_ITER`       | Number of maximum iterations                                                                                    | 1000        |
# | `--chrom CHROM [CHROM ...]` | The chromosome on which the model is fitted                                                                  | Iterating through 22 autosomes |
# | `--beta_std [BETA_STD]` | Boolean. If True, return standardized posterior SNP effect sizes                                                | False       |
# | `--thres THRES`         | Threshold value for convergence                                                                                 | 0.001       |
# | `--ignore_blk [IGNORE_BLK]` | Boolean. If false, elements categorized as same set but in different ld will be treated as different sets  | False       |
# | `--verbose [VERBOSE]`   | Boolean                                                                                                         | False       |
# | `--fixtau [FIXTAU]`     | Boolean. If false, will estimate tau in algorithm, otherwise will fix at 1                                       | False       |
# | `--flip [FLIP]`         | Boolean. If false, signs of beta in summary statistics will not be flipped when there's a switch in A1 and A2 alleles | False |
# 
# 
# 
# 
...
# 
# Here you can download the reference panels used by PRScs and information is taken from the PRScs GitHub: [PRScs GitHub](https://github.com/getian107/PRScs).
# 
# ### LD Reference Panels
# 
...
create_directory(folddirec+os.sep+"PRSbils")
                 ...
                 for ignore_blk in ignore_blks:
                  for fixtau in fixtaus:
                   for flip in flips:       
                    ...
                    va,vb,vc,vd,ve,vf,num_iter,beta_std,threshold,ignore_blk,fixtau,flip, str(p1_val), str(p2_val), str(p3_val), str(c1_val), str(c2_val), str(c3_val), "PRSbils", pvaluefile)
...
# 
# 1. `SampleData1/Fold_0/PRScs/Results.csv`
# 2. `SampleData1/Fold_1/PRScs/Results.csv`
# 3. `SampleData1/Fold_2/PRScs/Results.csv`
# 4. `SampleData1/Fold_3/PRScs/Results.csv`
# 5. `SampleData1/Fold_4/PRScs/Results.csv`
# 
