...
# # CTPR
# [CTPR (Cross-Trait / Cross-eThnic Penalized Regression)](http://lianglab.rc.fas.harvard.edu/CTPR/) software was originally developed for multi-trait polygenic risk prediction in large cohorts.
# 
#  
# ## Installation
# 
# Kindly follow the tutorial on [CTPR User Manual](https://github.com/wonilchung/CTPR/wiki/CTPR-User-Manual). We used the single-node version of the program rather than the MPI version.
# 
# ### Steps:
# 
# ```bash
...
# 
# [GitHub Repository for CTPR](https://github.com/wonilchung/CTPR/)
# 
# ## Hyperparameters
# 
...
# | `--separ-ind` or `--separate-individual [num,num,..]` | Specify the numbers of individuals for each phenotype separated by comma (,) in case of multiple phenotypes |
# | `--penalty` or `--penalty-term [num]`      | Specify the sparsity and cross-trait penalty terms (default 1; 1: Lasso+CTPR; 2: MCP+CTPR)                  |
# | `--nfold` or `--number-fold [num]`         | Specify the number of folds for coordinate descent algorithm (default 5)                                    |
# | `--prop` or `--proportion [num]`           | Specify proportion of maximum number of non-zero beta (default 0.25)                                        |
# | `--lambda2` or `--lambda2-option [num]`    | Specify value for lambda2. If negative value is specified, pre-specified values are used for lambda2 (default -3) |
