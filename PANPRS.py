...
# # PANPRS
# 
# In this notebook, we will use PANPRS to calculate the PRS. For more details, you can visit the [PANPRSnext GitHub repository](https://github.com/cran/PANPRSnext) and the [PANPRSnext documentation](https://cran.r-project.org/web/packages/PANPRSnext/PANPRSnext.pdf).
# 
# PANPRS is a shrinkage estimator for polygenic risk prediction (PRS) models based on summary statistics of genome-wide association (GWA) studies. It is based on the methods and original 'PANPRS' package as described in: Chen, Chatterjee, Landi, and Shi (2020) [doi:10.1080/01621459.2020.1764849](https://doi.org/10.1080/01621459.2020.1764849).
# 
# ## Installation
...
# 
# - **Multiple GWAS Information:** The function supports passing multiple GWAS information and uses z-statistics, which can be computed as follows:
#   $$
#   Z = \frac{\beta}{SE}
#   $$
# 
...
# |-------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
# | `summary_z`       | A matrix of summary statistics for each SNP and trait.                                                                                                 |
# | `n_vec`           | A vector of sample sizes for each of the Q traits corresponding to the Q columns of `summary_z`.                                                    |
# | `plinkLD`         | A matrix of LD values for each pair of SNPs.                                                                                                           |
# | `n_iter`          | The number of iterations to run the algorithm.                                                                                                        |
# | `upper_val`       | The upper bound for the tuning parameter.                                                                                                             |
# | `breaking`        | The number of iterations to run before checking for convergence.                                                                                      |
# | `z_scale`         | The scaling factor for the summary statistics.                                                                                                        |
# | `tuning_matrix`   | A matrix of tuning parameters.                                                                                                                         |
# | `tau_factor`      | A vector of factors to multiply the median value by to get the tuning parameters.                                                                     |
# | `len_lim_lambda`  | The number of tuning parameters to use for the first iteration.                                                                                       |
# | `sub_tuning`      | The number of tuning parameters to use for the second iteration.                                                                                     |
# | `lim_lambda`      | The range of tuning parameters to use for the first iteration.                                                                                       |
# | `len_lambda`      | The number of tuning parameters to use for the second iteration.                                                                                     |
# | `df_max`          | The maximum degrees of freedom for the model.                                                                                                         |
# | `sparse_beta`     | Whether to use the sparse version of the algorithm.                                                                                                  |
# | `debug_output`    | Whether to output the tuning combinations that did not converge.                                                                                     |
# | `verbose`         | Whether to output information through the evaluation of the algorithm.                                                                               |
# 
# ## PANPRS.R file
# 
