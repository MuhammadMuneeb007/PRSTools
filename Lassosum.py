...
# # Lassosum
# 
# In this notebook, we will use Lassosum to calculate the PRS.
# 
# **Note:** Lassosum needs to be installed using R.
# 
# We recommend using Linux.
# 
# Install Lassosum on R using the following command:
# 
# ```r
...
# ```
# 
# Lassosum calculates the correlation between the P-values and BETAS, and then uses the individual genotype data to calculate the new BETAS:
# 
# ```r
# cor <- p2cor(p = ss$P,
#              n = ss$N,
...
# ```
# 
# **Note:**  For both binary and continuous phenotype, BETAS were passed to the lassosum pipeline. For binary phenotype, convert the OR to betas, and when Lassosum calculates the new BETAS for multiple values of lambda and delta, we convert the new betas to OR using `np.exp(df["BETA"])` for binary phenotype.
# 
# 
# 
# Specify the lambda and delta in the `Lassosum.R` file:
# 
# ```r
# delta <- c(0.1, 0.2)
...
# ```
# 
# `Lassosum.R` accepts the following parameters.
# 
# **Note:** Argument Descriptions:
# 1. Directory
# 2. File name
# 3. Output file name
...
# Save this file as it will be used to by Plink for clumping
df.to_csv(filedirec + os.sep +filedirec+".txt",sep="\t",index=False)


df.to_csv(filedirec + os.sep +filedirec+"_lassosum.txt",sep="\t",index=False)
...
# ## GWAS file processing for Lassosum.
#  **GWAS format** - The GWAS should be in the above mentioned format, and the lassosum code in R will automatically transform the file in the required format. 
#  
# ```R
...
def fit_binary_phenotype_on_PRS(traindirec, newtrainfilename,p,m,ref, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile,lasso_lambda,lasso_delta):
...
# This function fit the binary model on the PRS.
def fit_continous_phenotype_on_PRS(traindirec, newtrainfilename,p,m,ref, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile,lasso_lambda,lasso_delta):
    threshold_values = allpvalues
...
def transform_lassosum_data(traindirec, newtrainfilename,p,m,ref, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile):
    ...
    files_to_remove = [
        traindirec+os.sep+"train_data.lassosum_betas",
        traindirec+os.sep+"train_data.lassosum_gwas",
        traindirec+os.sep+"train_data.lassosum_parameters"
    ...
    # Define the pattern for files to remove
    file_pattern = traindirec + os.sep + "GWAS_LASSOSUM_*"
    ...
    os.system("Rscript Lassosum.R "+os.path.join(filedirec)+"  "+traindirec+" "+trainfilename+" "+newtrainfilename+".clumped.pruned"+" "+p+" "+m+ " "+ ref)
    
    # After obtaining betas from the Lassosum
    # Append the new betas.
    # Read the Lassosum parameters. Specify the Lambda and delta in the Lassosum.R file.
    #Specify the lambda and delta in the `Lassosum.R` file:
    ...
    # Read the parameters.
    gridparameters = pd.read_csv(traindirec+os.sep+"train_data.lassosum_parameters",sep=",")
    # Read the new betas generated by Lassosum.
    allbetas = pd.read_csv(traindirec+os.sep+"train_data.lassosum_betas",sep=",")
    # Replace na with 0
    allbetas = allbetas.fillna(0)

    
    for index, row in gridparameters.iterrows():
        # Read the original gwas file  in the first step.
        gwas = pd.read_csv(traindirec+os.sep+"train_data.lassosum_gwas",sep=",")
        ...
        gwas.replace([-np.inf, np.inf], 0, inplace=True)
        gwas.fillna(0, inplace=True)          
        
        # Save the GWAS file.
        gwas[["SNP","A1","newbetas"]].to_csv(traindirec+os.sep+"GWAS_LASSOSUM_"+str(lambda1)+"_"+str(delta1),sep="\t",index=False)
    
        command = [
            "./plink",
             "--bfile", traindirec+os.sep+newtrainfilename+".clumped.pruned",
            ### SNP column = 1, Effect allele column 2 = 4, Effect column=4
            "--score", traindirec+os.sep+"GWAS_LASSOSUM_"+str(lambda1)+"_"+str(delta1), "1", "2", "3", "header",
        ...
        if check_phenotype_is_binary_or_continous(filedirec)=="Binary":
            ...
            fit_binary_phenotype_on_PRS(folddirec, newtrainfilename, p, m,ref,str(p1_val), str(p2_val), str(p3_val), str(c1_val), str(c2_val), str(c3_val), "lassosum", pvaluefile,row['lambda'] ,row['delta'])
        else:
            print("Continous Phenotype!")
            fit_continous_phenotype_on_PRS(folddirec, newtrainfilename, p, m,ref,str(p1_val), str(p2_val), str(p3_val), str(c1_val), str(c2_val), str(c3_val), "lassosum", pvaluefile,row['lambda'] ,row['delta'])
...
#referencepanelsnames = ["same"]

result_directory ="lassosum" 
      ...
      for p in numberofpca:
       for m in models:
        for ref in referencepanelsnames:
         transform_lassosum_data(folddirec, newtrainfilename, p, m,ref,str(p1_val), str(p2_val), str(p3_val), str(c1_val), str(c2_val), str(c3_val), result_directory, pvaluefile)
...
# ```python
# # foldnumber = sys.argv[1]
# python Lassosum.py 0
# python Lassosum.py 1
# python Lassosum.py 2
# python Lassosum.py 3
# python Lassosum.py 4
# ```
# 
# The following files should exist after the execution:
# 
# 1. `SampleData1/Fold_0/lassosum/Results.csv`
# 2. `SampleData1/Fold_1/lassosum/Results.csv`
# 3. `SampleData1/Fold_2/lassosum/Results.csv`
# 4. `SampleData1/Fold_3/lassosum/Results.csv`
# 5. `SampleData1/Fold_4/lassosum/Results.csv`
# 
