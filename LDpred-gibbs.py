...
# ```
# 
# The `ldpred gibbs` command is used to perform Gibbs sampling for LDpred. Below are the options available:
# 
# ```plaintext
...
# | `--ldf LDF`             | LD file (prefix). A path and filename prefix for the LD file. If it does not exist, it will be generated. This can take up to several hours, depending on LD radius used. |
# | `--out OUT`             | Output Prefix for SNP weights.                                              |
# | `--f F [F ...]`         | Fraction of causal variants used in the Gibbs sampler.                       |
# | `--N N`                 | Number of individuals on which the summary statistics are assumed to be based. |
# | `--n-iter N_ITER`       | The number of iterations used by the Gibbs sampler. The default is 60.       |
# | `--n-burn-in N_BURN_IN` | The number of burn-in iterations used by the Gibbs sampler. The default is 5. |
# | `--h2 H2`               | The genome-wide heritability assumed by LDpred, which is then partitioned proportional to the number of SNPs on each chromosome. By default, it estimates the heritability for each chromosome from the GWAS summary statistics using LD score regression. |
# | `--use-gw-h2`           | Estimate heritability genome-wide and partition it proportional to the number of SNPs on each chromosome instead of estimating it for each chromosome separately. This is generally recommended if the summary statistics are based on small sample sizes (approx <50K), or if the trait is not very heritable. |
...
# | `--hickle-ld`           | Use hickle instead of pickle for storing LD files. This saves memory, but generally takes more time to write and load. Requires hickle to be installed on your system. |
# | `--incl-long-range-ld`  | Includes SNPs that are located in long-range LD regions in the Gibbs sampler. The LD-pred inf effect estimates are used for these by default. |
# 
# 
# ## References
# 
...
# 
# 1. `SampleData1/Fold_0/LDpred-gibbs/Results.csv`
# 2. `SampleData1/Fold_1/LDpred-gibbs/Results.csv`
# 3. `SampleData1/Fold_2/LDpred-gibbs/Results.csv`
# 4. `SampleData1/Fold_3/LDpred-gibbs/Results.csv`
# 5. `SampleData1/Fold_4/LDpred-gibbs/Results.csv`
# 
