...
# # PRScs
# 
# In this notebook, we will use PRScs to calculate the PRS.
# 
# PRScs is a Python based command line tool that infers posterior SNP effect sizes under continuous shrinkage (CS) priors using GWAS summary statistics and an external LD reference panel: [PRScs GitHub](https://github.com/getian107/PRScs).
#  
...
# 
# 
# ## GWAS File Processing for PRSCS for Binary Phenotypes
# 
...
df_transformed.to_csv(filedirec + os.sep +filedirec+".PRSCS",sep="\t",index=False)
...
def transform_prscs_data(traindirec, newtrainfilename,numberofpca,ref_dir,phi,va,vb,number_of_iteration,burning_iteration,thining_iteration, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile):
    ...
    import glob
    file_list = glob.glob(traindirec+os.sep+"PRSCS_pst_eff*.txt")
    ...
    files_to_remove = [
        traindirec+os.sep+".PRSCS_final_GWAS",
    ]
    ...
    if phi=="auto":
        command = [
            ...
            "--sst_file=" + filedirec + os.sep +filedirec+".PRSCS",
            ...
            "--out_dir=" + traindirec+os.sep+"PRSCS"
        ]
        print(" ".join(command))
        subprocess.run(command)
    else: 
    ...
    # PRScs generates file for each CHR, access it and sum it.
    ...
    merged_df[["SNP","A1","NewBeta"]].to_csv(traindirec+os.sep+".PRSCS_final_GWAS",sep="\t",index=False)
    ...
    command = [
        ...
        "--score", traindirec+os.sep+".PRSCS_final_GWAS", "1", "2", "3", "header",
