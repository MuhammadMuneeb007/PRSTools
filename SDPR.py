...
# 
# - [MHC Included](https://drive.google.com/file/d/1BhZdUNG9GfCaeH3m0Ou7yE2Ljvp39-j7/view?usp=sharing)
# - [MHC Removed](https://drive.google.com/file/d/1o-cqLDHfqouribbzlEjfM0ikedQ9H4jc/view?usp=sharing)
# 
...
# 
# The `-ss` option (required for `-make_ref`) specifies the path to the summary statistics file. It is recommended to follow the pipeline to clean up summary statistics using `munge_sumstats.py`. 
# 
...
# | `-mcmc`      | Perform MCMC.                                                                  | -        |
# | `-ss`        | Path to the summary statistics file (required for `-make_ref`).              | -        |
# | `-ref_prefix`| Path to the prefix of the bim file for the reference panel (required for `-make_ref`). | -        |
# | `-ref_dir`   | Path to the directory containing the reference LD information (required).     | -        |
# | `-valid`     | Path to the bim file for the testing dataset (optional).                       | -        |
# | `-out`       | Path to the output file containing estimated effect sizes (required for `-mcmc`). | -        |
# | `-chr`       | Chromosome to work on (required). Currently supports 1-22.                    | -        |
# | `-N`         | GWAS sample size (required for `-mcmc`).                                       | -        |
# | `-M`         | Max number of variance components. Must be greater than 4.                    | 1000     |
# | `-opt_llk`   | Which likelihood to evaluate. `1` for vanilla modified likelihood and `2` for SNPs genotyped on different individuals. | 1        |
# | `-iter`      | Number of iterations for MCMC.                                                | 1000     |
# | `-burn`      | Number of burn-in iterations for MCMC.                                         | 200      |
# | `-thin`      | Thinning for MCMC.                                                              | 1        |
# | `-n_threads` | Number of threads to use.                                                       | 1        |
# | `-r2`        | r2 cut-off for partition of independent blocks.                                | 0.1      |
# | `-a`         | Factor to shrink the reference LD matrix.                                      | 0.1      |
# | `-c`         | Factor to correct for the deflation.                                           | 1        |
# | `-a0k`      | Hyperparameter for inverse gamma distribution.                                 | 0.5      |
# | `-b0k`      | Hyperparameter for inverse gamma distribution.                                 | 0.5      |
# | `-h`         | Print the options.                                                             | -        |
#  
# 
#  
