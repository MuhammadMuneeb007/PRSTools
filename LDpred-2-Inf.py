...
# # LDpred-2-Inf
# 
# In this notebook, we will use LDpred-2-Inf to calculate the Polygenic Risk Score (PRS).
# 
...
#   newgwas$newbeta <-beta_inf
#   result <-paste(".",args[2],paste(args[3],toString(".ldpred_inf"), sep = ""),sep="//")
...
def fit_binary_phenotype_on_PRS(traindirec, newtrainfilename,p , p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile,models,heritability,numberofvariants):
...
# This function fit the binary model on the PRS.
def fit_continous_phenotype_on_PRS(traindirec, newtrainfilename,p , p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile,models,heritability,numberofvariants):
    threshold_values = allpvalues
...
# ## Execute LDpred-2-Inf

# In[4]:


def transform_ldpred2_inf_data(traindirec, newtrainfilename,models,p, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile):
    ...
    files_to_remove = [
        ...
        traindirec+os.sep+"train_data.ldpred_inf",
        traindirec+os.sep+"train_data.ldpred_inf_gwas",
    ...
    if models=="LDpred-2_full":
        try:
            os.system("Rscript LDpred2-inf.R "+os.path.join(filedirec)+"  "+traindirec+" "+trainfilename+" "+newtrainfilename+".clumped.pruned"+ " "+"3"+" "+c3_val+" "+c1_val+" "+c2_val+" "+p)
    ...
    if models=="LDpred-2_hapmap":
        try:
            os.system("Rscript LDpred2-inf.R "+os.path.join(filedirec)+"  "+traindirec+" "+trainfilename+" "+newtrainfilename+".clumped.pruned"+ " "+"2"+" "+c3_val+" "+c1_val+" "+c2_val+" "+p)
    ...
    temp = pd.read_csv(traindirec+os.sep+"train_data.ldpred_inf",sep="\s+" )
    print(temp.head())
    ...
    temp.rename(columns={'rsid.ss': 'SNP', 'a0': 'A1','newbeta':'BETA'}, inplace=True)

    temp[["SNP","A1","BETA"]].to_csv(traindirec+os.sep+"train_data.ldpred_inf_gwas",sep="\t",index=False)
    
    command = [
        "./plink",
         "--bfile", traindirec+os.sep+newtrainfilename+".clumped.pruned",
        ### SNP column = 1, Effect allele column 2 = 4, Effect column=4
        "--score", traindirec+os.sep+"train_data.ldpred_inf_gwas", "1", "2", "3", "header",
    ...
    if check_phenotype_is_binary_or_continous(filedirec)=="Binary":
        ...
        fit_binary_phenotype_on_PRS(traindirec, newtrainfilename,p, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile,models,heritability,numberofvariants)
    else:
        print("Continous Phenotype!")
        fit_continous_phenotype_on_PRS(traindirec, newtrainfilename,p, p1_val, p2_val, p3_val, c1_val, c2_val, c3_val,Name,pvaluefile,models,heritability,numberofvariants)
     ...
     for c3_val in clump_kb:
      for p in numberofpca:
       for model in  h2models:
        transform_ldpred2_inf_data(folddirec, newtrainfilename,model, p, str(p1_val), str(p2_val), str(p3_val), str(c1_val), str(c2_val), str(c3_val), result_directory, pvaluefile)
...
# ```python
# # foldnumber = sys.argv[1]
# python LDpred-2-Inf.py 0
# python LDpred-2-Inf.py 1
# python LDpred-2-Inf.py 2
# python LDpred-2-Inf.py 3
# python LDpred-2-Inf.py 4
# ```
# 
# The following files should exist after the execution:
# 
# 1. `SampleData1/Fold_0/ldpred2_inf/Results.csv`
# 2. `SampleData1/Fold_1/ldpred2_inf/Results.csv`
# 3. `SampleData1/Fold_2/ldpred2_inf/Results.csv`
# 4. `SampleData1/Fold_3/ldpred2_inf/Results.csv`
# 5. `SampleData1/Fold_4/ldpred2_inf/Results.csv`
# 
